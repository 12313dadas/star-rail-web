        ...
        <div className="card overflow-x-auto">
          {tab === 'posts' && (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-400 border-b border-white/10"><th className="pb-2">标题</th><th>状态</th><th></th></tr></thead>
              <tbody>
                {posts.map((p) => (
                  <tr key={p.id} className="border-b border-white/5">
                    <td className="py-2">{p.title}</td>
                    <td>{p.published ? '已发布' : '草稿'}</td>
                    <td><button onClick={() => deletePost(p.id)} className="text-red-400 hover:underline">删除</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {tab === 'moments' && (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-400 border-b border-white/10"><th className="pb-2">内容</th><th></th></tr></thead>
              <tbody>
                {moments.map((m) => (
                  <tr key={m.id} className="border-b border-white/5">
                    <td className="py-2 max-w-md truncate">{m.content}</td>
                    <td><button onClick={() => deleteMoment(m.id)} className="text-red-400 hover:underline">删除</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {tab === 'music' && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500 mt-4">通过 API POST /api/music 添加曲目</p>
            </div>
          )}
        </div>
      )}
